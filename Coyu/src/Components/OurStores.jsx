...
const StoreCard = ({ image, mallName, address, timing }) => (
  <div className="flex-shrink-0 w-full max-w-xs mr-2 py-4 border rounded-lg shadow-md bg-white">
    ...
    <h3 className="font-semibold px-3 text-lg">{mallName}</h3>
    <p className="text-gray-500 px-3">{address}</p>
    <p className="text-gray-400 px-3">{timing}</p>
    <div className="text-blue-600 px-3 font-semibold mt-2">VIEW DETAILS &rarr;</div>
  </div>
);
...
const OurStores = () => {
  ...
  const stores = {
    ...
    Gurgaon: {
      ...
      mallName: 'Ambience Mall, Gurgaon',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-lg font-bold mb-4">OUR STORES</h2>
      ...
      <div className="flex space-x-4 mb-6">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-3 py-2 text-lg rounded-lg flex items-center border border-black font-semibold ${
